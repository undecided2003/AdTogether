      ...
      if (id) {
        try {
          ...
          if (adSnap.exists()) {
            ...
            setClickUrl(data.clickUrl || "https://");
            
            const prevCountries = data.targetCountries || (
              data.targetCountry && data.targetCountry !== "global" 
                ? [data.targetCountry] 
                : COUNTRIES.map(c => c.code)
            );
            setTargetCountries(prevCountries);
            
            setAdType(data.adType || "banner");
  ...
  return (
    <div className="flex-grow w-full max-w-4xl mx-auto py-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-3xl p-8 md:p-12 relative overflow-hidden shadow-2xl">
        ...
        <h1 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2 relative z-10">Edit Campaign</h1>
        <p className="text-zinc-500 dark:text-zinc-400 mb-10 relative z-10">Update your campaign's creative, copy, and target audience.</p>
